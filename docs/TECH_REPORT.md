...
- **Text frontend:** `prep_text.py` — `model_text` / `model_text_sandhi` return `(kannada_text, accent_array)`; Vedic marks stripped before Kannada route and remapped through sandhi. `render_plan.plan_render` = GPU-free noop manifest. Audio prosody remains classical (no synthesized svarita/anudātta).
...
- **Vedic svara routing (future-proofing):** combining marks U+0951 (RV saṃhitā *svarita* stroke; Unicode name UDATTA), U+0952 (anudātta), U+0953/U+0954, and Vedic Extensions U+1CD0–U+1CFF are removed before transliteration so IndicF5 never sees OOV codepoints. Unmarked syllables = implicit udātta (`None`). `accent_array` is 1:1 with `chandas_labeler.scan` / `syllabify_slp1` (`ॐ` → two slots for SLP1 `AUM`). Sandhi/echo/F→rU remaps labels via `remap_accents_slp1` (mid-string merges keep accents on the surviving nucleus). **Marks do not drive F0** — production audio is still classical śāstra chant; use `render_plan.plan_render` / `scripts/dry_run_render.py` for noop inspection. Demo **Plan only** skips the daily render quota.
